...
bool GLBLoader::load(const char* path) {
    ...
    for (size_t i = 0; i < model.nodes.size(); i++) {
        ...
        Node node; node.index = i; node.name = n.name; node.meshIndex = n.mesh; node.skinIndex = n.skin;
    ...
    for (size_t i = 0; i < nodes.size(); i++) { if (nodes[i].parent == -1) rootNodes.push_back(i); }
    ...
    for (auto& mesh : model.meshes) {
        for (auto& primitive : mesh.primitives) {
            ...
            p.indexCount = idxAcc.count; p.indexType = idxAcc.componentType;
